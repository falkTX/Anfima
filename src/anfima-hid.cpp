...
static constexpr const uint8_t MSG_START_len = sizeof(MSG_START);
static constexpr const uint8_t MSG_END_len = sizeof(MSG_END);
...
HID* anfima_hid_init(void (*const callback)(void* arg, const char* msg), void* const arg)
{
    ...
    {
        std::wcstombs(serial, info->serial_number, SERIAL_NUMBER_LEN);
    ...
    std::memcpy(hid->serial, serial, SERIAL_NUMBER_LEN);
...
const char* anfima_hid_serial_number(const HID* const hid)
{
    return hid->serial;
}

const char* anfima_hid_error(const HID* const hid)
{
    if (! _hid_connected)
        return nullptr;
...
static void _hid_send(HID* const hid, const char* const msg)
{
    ...
    for (size_t msglen = std::strlen(msg), msgoff = 0; !done;)
    {
        ...
        if (msgoff == 0)
        {
            std::memcpy(report + written, MSG_START, MSG_START_len);
            written += MSG_START_len;
        }
        ...
        if (written == REPORT_SIZE)
        {
            ...
            {
                fprintf(stderr, "failed to write HID message: %ls | w %d\n", hid_error(hid->device), w);
                break;
            }
        ...
        if (written + MSG_END_len <= REPORT_SIZE)
        {
            std::memcpy(report + written, MSG_END, MSG_END_len);
            written += MSG_END_len;
            done = true;
        }
        ...
        {
            fprintf(stderr, "failed to write final HID message: %ls | w %d\n", hid_error(hid->device), w);
            break;
        }
...
static void* _hid_thread(void* const arg)
{
    ...
    static constexpr const char* errorMarker = "ERROR-MISSING-DSTART-SEQUENCE";
    ss << errorMarker;

    const auto clear = [hid, &ss, &start, &end]{
        ss = {};
        ss << errorMarker;
        start = end = -1;
