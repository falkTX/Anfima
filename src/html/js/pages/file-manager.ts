...
import { Category, OpenFileReceiverMessage_File, RemoveUserFileMessage } from '../types/backend-data.js';
...
const initFileManager = async (backend: Backend, cloud: Cloud, htmlElements: HTMLElements, logger: Logger, wasm: Wasm) => {
    logger.info('Requesting user file list...');

    const elems = htmlElements.pages.fileManager;

    const userFiles = Object.fromEntries(
        (await backend.findUserFiles()).payload.data.map((e) => { return [e.id, e] })
    );
...
Updated at: ${dateToString(userFile.updated_at)}`);
    ...
    elems.addButton.onclick = (ev: PointerEvent) => {
        ...
        case 'cabinets':
            if (! wasm.initialized) {
                logger.useReportElem();
                logger.error(wasm.error || 'wasm is not initialized yet');
                return;
            }
    ...
    elems.addModal.container.onsubmit = async (ev: SubmitEvent) => {
        ...
        for (let i = 0; i < files.length; ++i) {
            ...
            try {
                ...
                const category: Category = dirName == 'neural-models'
                                         ? (fileElems[i].querySelector('select[name="category"]') as HTMLSelectElement).value as Category
                                         : '';
                const userFile: OpenFileReceiverMessage_File['payload'] = {
                    category,
                    createdAt: date,
                    dirName,
                    fileName: file.name,
                ...
                let payload: Uint8Array | undefined = new Uint8Array(await file.arrayBuffer());

                payload = wasm.procfile(payload);
                if (! payload) {
    ...
    elems.removeModal.onsubmit = async (ev: SubmitEvent) => {
        ...
        try {
            ...
            for (let i = 0; i < ids.length; ++i) {
                ...
                const payload: RemoveUserFileMessage['payload'] = {
                    dirName: userFile.dir_name,
                    fileName: userFile.file_name,
                    id: userFile.id,
                };

                if (ids.length > 5) {
                    logger.info(`Deleting file ${ i + 1 }/${ ids.length }...`);
                }

                await backend.removeUserFile(payload);
